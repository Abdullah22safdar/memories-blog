        ...
        if (post.like.length > 0)
            return post.like.find((like) => like === (user?.result?.googleId || user?.result?._id))
                ? (<>
                    ...
                    &nbsp;{post.like.length > 2 ? `You and${post.like.length - 1}others` :
                    `${post.like.length}like${post.like.length > 1 ? 's' : ''}`}
                </>) : (
                    <><ThumbUpAltOutlined
                        fontSize="small"/>&nbsp;{post.like.length} {post.like.length === 1 ? 'Like' : 'likes'}</>
                );
